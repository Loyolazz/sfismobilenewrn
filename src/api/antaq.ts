...
function explainAxiosError(err: unknown, tag = "ERR") {
  ...
  // Muitos “Network Error” no Android trazem detalhe útil aqui:
  // @ts-ignore
...
async function axiosSoapPost(action: string, xmlBody: string): Promise<string> {
  ...
  try {
    ...
    // 2) Fallback SOAP 1.1 – proteger com try/catch também
    try {
...
export async function soapRequest(action: string, params?: Record<string, any>) {
